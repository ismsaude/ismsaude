                ...
                <div className="w-full max-w-[800px] mx-auto grid grid-cols-1 sm:grid-cols-2 gap-8 xl:gap-12">
                    {modules.map((mod, idx) => (
                        ...
                        >
                            ...
                            <div className={`w-20 h-20 mb-6 rounded-[2rem] flex items-center justify-center ${mod.iconBg} shadow-inner transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3`}>
                                <mod.icon size={36} className={mod.iconColor} strokeWidth={2.5} />
                            </div>

                            <h3 className="text-xl xl:text-2xl font-black text-slate-900 drop-shadow-none mb-2 leading-tight tracking-normal z-10">{mod.title}</h3>
                            <p className="text-xs xl:text-sm font-bold text-slate-500 px-8 leading-snug z-10 uppercase tracking-widest flex items-start justify-center h-10">{mod.desc}</p>
                        </div>
                    ))}
                </div>
